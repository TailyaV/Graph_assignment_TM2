...
Graph Algorithms::bfs(const Graph& g, int i){
    ...
    while (!queue.isEmpty())
    {
        ...
        for(int j = 0; j < x -1; j++){
            if(vartaxArr[curr->getValue() -1][j] > 0 && visited[vartaxArr[curr->getValue() - 1][j] - 1] == false){
                ...
                result.addEdge(curr->getValue(), vartaxArr[curr->getValue()-1][j]);
            }
        }
        delete curr;
    }
...
Graph Algorithms::dfs(const Graph& g, int i){
    if(g.getVartexNum() < i || i < 0){
        throw std::invalid_argument("The start vartex is not exist");
    }
    ...
    while (!stack.isEmpty())
    {
        ...
        if(lastVal != 0){
            result.addEdge(lastVal, curr->getValue());
        }
...
Graph Algorithms::dijkstra(const Graph& g, int i){
        ...
        for(int k = 0; k < x - 1; k++){
            ...
            if(currNeighbor > 0 && visited[currNeighbor - 1] == false){
                ...
                if(newWeight < vartaxDist[currNeighbor - 1]){
                    ...
                    prev[currNeighbor - 1] = curr->getValue();
    ...
    for(int r = 0; r < x; r++){
        std::cout <<"shortest dist to " << r + 1 << " is " << vartaxDist[r] << std::endl;
    }
    std::cout << std::endl;
    for (int v = 0; v < x; v++) {
        if (prev[v] != -1) {
            int u = prev[v];
            int weight = g.getWeight(v + 1, u);
            result.addEdge(u, v + 1, weight);
        }
    }
    return result;
}
...
Graph Algorithms::prim(const Graph& g){
    ...
    std::cout << "Prim start vartax is: " << graphSize << " finding MST:" << std::endl;
...
Graph Algorithms::kroskal(const Graph& g){
    std::cout << "Started Kruskal Algorithm, finding MST: " << std::endl;
    int** neigh = g.getNeighbors();
    int graphSize = g.getVartexNum();
    for(int i = 0;i < graphSize; i++){
        int flag = 0;
